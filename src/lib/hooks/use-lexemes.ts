...
import type { LexemesListParams } from "@/lib/types/api";

export const lexemeKeys = {
  all: ["lexemes"] as const,
  lists: () => [...lexemeKeys.all, "list"] as const,
  list: (params: LexemesListParams) => [...lexemeKeys.lists(), params] as const,
  detail: (lexemeId: string) => [...lexemeKeys.all, lexemeId] as const,
};

export function useLexemes(params: LexemesListParams) {
  return useQuery({
